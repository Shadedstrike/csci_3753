...
static char device_buffer[BUFFER_SIZE]; //create the buffer
...
struct file_operations pa2_file_operator{
	      .owner = THIS_MODULE,
...
{
	int current_pos; //hold the current position
	...
	if (/* condition */) { //if user attempts to seek before or after the file
		printk(KERN_ALERT "You cannot access the file outside it's bounds!!! \n");
	}
	printk(KERN_ALERT "The current offset in this seek is: %d \n", offset);
...
static int pa2_char_driver_init(void)
{
	...
	/* register the device */
	register_chrdev(62, "pa2_char_driver", &pa2_file_operator)

	/* allocate memory to store user data */
	void* kmalloc(size_t 1024, gfp_t GFP_KERNEL)
